...
class UserClassificationFetchController extends Controller
{
    /**
    ...
    {
        $this->class = new UserClassification;
    }   
    ...
    protected function formatItem($item)
    {
        return [
            'id' => $item->id,
            'name' => $item->name,
            'categories' => $item->renderFormattedAccessForTable(),
    ...
    public function fetchItem($id = null) {
        ...
        $categories = ConfidentialityCategory::all();

        if ($id) {
            $item = UserClassification::withTrashed()->findOrFail($id);
            $item->categories = $item->accesses->pluck('id')->toArray();
