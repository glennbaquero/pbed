...
class ConfidentialityCategoriesTableSeeder extends Seeder
{
    /**
    ...
    {
        $row = 0;
        if(($handle = fopen('database/csv/ConfidentialityCategories.csv', "r")) !== FALSE){ // Check if CSV file exists
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) { // Parse data inside the file
                if ($row > 0) { // row check
                    ...
                    $item = new ConfidentialityCategory(); 
                    $item->name = $data[0]; 
                    $item->save();
                }
                $row++;
