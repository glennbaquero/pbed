	...
	<div>
        <filter-box @refresh="fetch">
            <template v-slot:left>
                <button class="btn frm-btn type-1 px-4 mr-2" v-if="showBatchDeletion" @click="showConfirmation">{{ batchButtonText }}</button>
            </template>
                ...
                <tr v-for="item in items" >
                    <td>
                        <label class="frm-check">
                            <input class="frm-form__check" v-model="item.selected" @change="getSelected" name="selected[]" type="checkbox">
...
export default {
    ...
    props: {
        ...
        batchUrl: String,
        batchButtonText: {
            default: 'BATCH DELETION',
            type: String
        },
        messageBody: {
            default: 'Do you want to permanently remove this set of information from your record?'
        },
        messageTitle: {
            default: 'Archive Record?'
        }
    },
