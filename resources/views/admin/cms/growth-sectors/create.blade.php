@extends('admin.master')

@section('meta:title', 'Create Growth Sectors of The Projects')

@section('content')

<!-- Content Wrapper. Contains page content -->
<div class="content-wrapper">
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h4 class="fntwght--semibold font-1">Create Growth Sectors of The Projects</h4>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="{{ route('admin.growth-sectors.index') }}">Growth Sectors of The Projects</a></li>
                    <li class="breadcrumb-item active"><a href="javascript:void(0)">Create</a></li>
                </ol>
            </div>
        </div>
    </section>

    <!-- Main content -->
    <section class="content">
        <div class="card bg--transparent">
            <div class="card-header">
                <div class="bg--gray">
                    <ul class="nav nav-pills">
                        <li class="nav-item"><a class="nav-link active" data-target="#tab1" href="javascript:void(0)" data-toggle="tab">Information</a></li>
                    </ul>
                </div>
            </div><!-- /.card-header -->
            <div class="card-body">
                <div class="tab-content">
                    <div class="tab-pane show active" id="tab1">
                        <project-priority-area-view
                        fetch-url="{{ route('admin.project-priority-areas.fetch-item') }}"
                        submit-url="{{ route('admin.growth-sectors.store') }}"
                        ></project-priority-area-view>
                    </div>
                </div>
            </div>
        </div>
    </section>
    {{-- <section class="content">
        <news-view
        fetch-url="{{ route('admin.news.fetch-item') }}"
        submit-url="{{ route('admin.news.store') }}"
        ></news-view>
    </section> --}}
</div>

@endsection